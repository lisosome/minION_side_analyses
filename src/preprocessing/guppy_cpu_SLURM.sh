...
BARCODE)
 ...
 module load ont-guppy-cpu/6.5.7
...
guppy_params="--compress_fastq --recursive --num_callers 12 --chunks_per_runner 208 --chunks_per_caller 10000 --barcode_kits ${kit} --enable_trim_barcodes --detect_barcodes --detect_mid_strand_barcodes"

guppy_basecaller -i ${baseout}/0.RAW.DATA -s ${baseout}/1.BASECALLING -c ${config} ${guppy_params}
;;
esac
